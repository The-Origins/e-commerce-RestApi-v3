...
productSchema.statics.findByName = async function (name) {
  ...
  let results = await this.find({ name: new RegExp(name, "i") }).sort({
    "rating.score": -1,
  });
  return results;
};
...
productSchema.statics.findByBrand = async function (brand) {
  ...
  for (let b in brands) {
    //check if the brand passed in the args contains any of the brands that exist e.g -> "dells".includes("dell") -> true
    ...
    if (brand.includes(brands[b])) {
      ...
      let update = await this.find({ brand: brands[b] });

      //filter out all the results that are already in the results array
      update = update.filter((item) => !results.some((result) => String(result._id) === String(item._id)))
...
productSchema.statics.findByCategory = async function (category) {
  ...
  for (let cat in categories) {
    ...
    if (category.includes(categories[cat])) {
      ...
      let update = await this.find({ categories: categories[cat] });
      
      //filter out results that already exist in the results array
      update = update.filter((item) => !results.some((result) => String(result._id) === String(item._id)))
...
productSchema.statics.findBySearch = async function (
  ...
  const updateResults = (updateArr) => {
    ...
    const output = updateArr.filter((element) => {
      return !results.some(
        (result) => String(result._id) === String(element._id)
      );
    });
  ...
  //filters
  //it is recomended that this is done on the front end
  //but this is basically all the logic needed
  if (maxPrice && minPrice) {
    ...
    results = results.filter((result) => {
      return (
        result.unitPrice.amount >= minPrice &&
        result.unitPrice.amount <= maxPrice
      );
    });
  }
    ...
    results = results.filter((result) => {
      ...
      for (let property in properties) {
        ...
        for (let productProperty in result.properties) {
          ...
          if (
            String(result.properties[productProperty].name) ===
            String(properties[property].name)
          ) {
            //if it does then only send it if the values are the same
            return (
              String(result.properties[productProperty].value) ===
