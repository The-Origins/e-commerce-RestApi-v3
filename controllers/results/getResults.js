...
module.exports = async (req, res, next) => {
  ...
  try {
    const results = await Product.findBySearch(
      String(queries.search).replace("+", " ").toLowerCase(),
      queries.max,
      queries.min,
      queries.brand,
      properties
    ...
    if (queries.limit) {
      return res.json({
        success: true,
        data: results.slice(0, Number(queries.limit)),
        message: `retrieved results for search '${queries.search}'${
          filters.length > 1 ? ` with filters ${filters}` : ``
        }`,
      });
    }
    return res.json({
      ...
      data: results,
      message: `retrieved results for search '${queries.search}'${
        filters.length > 6 ? ` with filters ${filters}` : ``
      }`,
    });
  } catch (error) {
    next(error);
